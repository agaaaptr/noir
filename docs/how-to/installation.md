...
| Path | What |
|---|---|
| `~/.noir/runtime/node/` | A pinned Node 22.x LTS runtime (used by the shim, isolated from your system Node). |
...
2. Provisions a managed Node 22.x runtime under `~/.noir/runtime/node/` (the shim uses it; no system Node prerequisite). If the runtime is already provisioned, this is a no-op.
...
```bash
noir install 1.5.0           # explicit pin (with a warning that it's older than 1.6.0)
noir install --spec 1.5.0
```
...
noir update --spec 1.6.0    # pin a specific version
```
