...
- **Host adapters are special.** A new host (OpenCode, Gemini, …) goes **inside `@noir-ai/adapters`**, not in a new package. Do not spawn `@noir-ai/opencode`, `@noir-ai/gemini`, etc.
- **Distribution artifacts are not packages.** The native installers (`scripts/install.sh`, `scripts/install.ps1`), the Homebrew formula (`packaging/homebrew/noir.rb`), and the Scoop manifest (`packaging/scoop/noir.json`) are release artifacts owned by the release workflow — not `@noir-ai/*` packages. They install the **published** `@noir-ai/cli` tarball; they do not get their own version. Refresh their `url`/`sha256`/`version` on each stable release (see [releasing.md §2d](releasing.md#2d-homebrew--scoop-stable-only-manual-bump-on-each-stable)).

If you find yourself reaching for a new package, check first that the capability doesn't already have a home: the package split was deliberate (see the [design blueprint](../internal/specs/2026-07-23-noir-toolkit-design.md)). The most recent addition was `@noir-ai/create` — added only when scaffolding graduated from a CLI-internal helper to a shared engine with its own public API and release contract.
