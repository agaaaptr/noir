...
| `@noir-ai/skills` | The native builtin skill pack (33 builtins + 1 integration = 34 skills) + a copy-and-validate compiler. Emits host-shaped `noir-*` artifacts: Claude `SKILL.md` files or Cursor `.mdc` rules. |
...
Built on one keystone refactor (`managedBlock` + shared `blockWriter` + `HostAdapter` emitters), seven capability slices first shipped together in `v1.2.0-beta.1` — K/R/I/P/S/X plus **S10 multi-host** and the **S11** SDK/`doctor` remainder:
...
- **S11** Distribution + SDK remainder — `docs/reference/packages.md` (the per-package framework/library API surface) and a `noir doctor` `publish` check (advisory package-metadata validation). (Distribution itself shipped at `1.0.0-beta.1`.)

Full design record: [`specs/2026-07-25-v1x-capabilities-design.md`](../internal/specs/2026-07-25-v1x-capabilities-design.md); per-slice specs under [`internal/specs/`](../internal/specs/); the locked decisions in [ADR-0003](../decisions/0003-v1x-capabilities.md) (K/R/I/P/S/X) and [ADR-0004](../decisions/0004-multi-host-adapters.md) (S10).
