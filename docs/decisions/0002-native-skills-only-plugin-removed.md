...
- **In-tree history trade-off.** Readers lose the plugin-era history in the tree (the `plugins/` directory, marketplace manifest, and pre-Noir specs). The durable rationale is retained here, in the design blueprint (`docs/specs/2026-07-23-noir-toolkit-design.md`, itself a dated design record), and in ADR-0001. The native skill pack carries forward the predecessor's best playbook content as original Noir re-implementations.
