...
- **Decisions of record:** `docs/decisions/` (ADR series — `0001`…`0011`).
...
- **Local daemon** — single-writer store, stdio + Streamable HTTP transports, read-only FS fallback, 17+ MCP tools. Per-project records at `~/.noir/daemons/<projectId>.json`, a configured `daemon.port` honoured as a preference, a `0600` bearer token enforced on the HTTP transport, and connect-first activation (ADR-0010).
...
- **v1.14.0 — daemon hardening + `noir init` completeness + `.noir/.env` precedence** (ADR-0010 + ADR-0011): per-project daemon records, a real `daemon.port`, an HTTP auth token, connect-first activation, `noir init`/`--upgrade` completeness, and the `.noir/.env` precedence inversion with provenance. — **IMPLEMENTED on `develop` 2026-09-11; release flow pending** (see the current-status block above).
