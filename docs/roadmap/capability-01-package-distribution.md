...
- **CLI self-update + migration** (`packages/cli/src/commands/{install,update}.ts`, `packages/core/src/install-*.ts`):
  ...
  - **Version-assert** — `noir install` refuses a downgrade and a below-floor version (per-segment numeric semver comparison) on every path, interactive included, with a usage error; `noir update` refuses a below-floor target the same way and never installs a downgrade (a target at or below the installed version is reported as `up to date` and exits 0). A positional version pin is installed only when it is at or above both the installed version and the `update.minVersion` floor.
