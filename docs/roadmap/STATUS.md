...
- **2026-08-04** — **1.7.3 bugfix line on `develop` (pending publish)**, from systematic root-cause debugging + a **23-agent pre-release audit workflow** (find→adversarial-verify, 5 bug-classes): the bundling `require()` class fix — `crypto` (`fix(create)` `0376dbb`) + `fs` (`fix(store,workflow)` `5e8def7`) latent `Dynamic require of X` crashes on the conflict/rename path; native-install `chmod +x` shim (the `noir update` → `permission denied` bug, `fix(install)` `6120bf1`) + spinner UX for install/update/daemon; **opencode `opts.command` threading** — the one adapter the 1.7.2 ENOENT fix missed (`fix(adapters,store)` `db99639`); **store `busy_timeout = 5000`** — defense vs `SQLITE_BUSY` when a second writer opens the same `.db`; table-driven cross-adapter command-threading parity test (+11 tests → 1439). Audit also confirmed: zero `__require` shims in any `.js` executable; all 17 MCP tool names pass the charset regex. **Windows native-install bugs deferred** (npm.exe/unzip/install.ps1 parity — pre-existing, need a Windows VM; CI has no Windows smoke).
...
- Cut the **1.7.3** patch release (beta on `develop`, then stable on `main`), then return to the **C2 TUI delta** (richer widgets, command palette).
