...
- **HTTP-transport auth token**: a fresh 32-byte token per start, written `0600` to `~/.noir/daemons/<scopeKey>.token` before the daemon serves (no 401 window on a fresh start), enforced on `/mcp` with a constant-time compare; `/health` stays token-free. For a project daemon the scope key is the projectId, so the token sits next to that project's record; a workspace daemon is keyed by workspace name — its token lives in the same `~/.noir/daemons/` directory, but its record is elsewhere, at `~/.noir/workspaces/<name>/daemon.json`. Clients read the 0600 file — never a config value ([token.ts](../../packages/daemon/src/token.ts), [workspace-record.ts](../../packages/daemon/src/workspace-record.ts), [http.ts](../../packages/daemon/src/http.ts)).
