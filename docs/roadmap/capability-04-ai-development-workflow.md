...
- **FSM lifecycle engine (v1.6.0):** intake → clarify → spec → plan → execute → verify → document, 9 states total, with observable, escapable gates at spec/plan/verify — `packages/workflow/src/engine.ts`, `packages/workflow/src/state-machine.ts`, `packages/workflow/src/gates.ts`.
...
- FSM runs intake → clarify → spec → plan → execute → verify → document with observable, escapable gates at spec/plan/verify. — **MET** (v1.6.0; see `packages/workflow/src/engine.ts`).
