...
- **Resolved in v1.9.4 (C3, 2026-08-10):** Skill pack curated 34→26 via 5 merges + gerund renames; all 26 builtins + 1 integration become full playbooks (zero stubs, WHAT+WHEN descriptions, follow-up guidance, host-tool maximization). Runtime-derived registry queryable via `noir skills registry --json`. Structural quality gate (`validateSkill` + `lintSkill` + `noir skills lint`). Offline evals harness (`evals/evals.json` + vitest runner, 2 shipped examples). ClickUp integration enhanced (STEP-0 auth gate, 12 API pitfalls, verb dispatch, attachment handling). SessionStart hook bootstrap at `noir init` + NOIR.md amplifier + router contract. Full gate green (1561 tests). K3 (skills-compiler generalization → `discoverIntegrations` + `integration.json`, landed in Slice X); R4/R5 (`rules:` config block + `noir doctor` RULES.md budget check); P3/P4 (`draftPrd` + `prd:` config + `advance()` soft PRD gate); Workflow dual-source-of-truth collapse (W1) + vestigial checkpoint (W2) + S4 nits (W3); Context kNN-only-hit snippet hydration (C1); Toolchain stale-skill-dir cleanup (T2) + `tsconfig.test.json` pilot (T1); lint → 0.
