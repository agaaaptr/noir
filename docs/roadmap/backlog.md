...
- **Resolved in the 2026-08-04 bugfix line (on `develop`, shipped in 1.7.3):** four post-1.7.2 fixes from systematic debugging + a 23-agent pre-release audit workflow — the bundling `require()` class fix (`crypto` in `create/scaffold.ts` `0376dbb`, `fs` in `store/markdown.ts` + `workflow/artifacts.ts` `5e8def7`; latent `Dynamic require of X` crashes on the conflict/rename path), **native-install `chmod +x` shim** (`installManagedNode` wrote `0o644`, never `chmod +x` → `noir update` produced a non-executable binary; `fix(install)` `6120bf1`) + spinner UX for install/update/daemon, **opencode `opts.command` threading** (the one adapter the 1.7.2 ENOENT fix missed; `fix(adapters,store)` `db99639`), and **store `busy_timeout = 5000`** (better-sqlite3 default is 0; defense vs `SQLITE_BUSY` when a second writer opens the same `.db`). Audit also added a table-driven cross-adapter command-threading parity test (+11 tests → 1439).
