...
> **As of 2026-07-26. v1.3.0-beta.5 PUBLISHED on npm (dist-tag `beta`); supersedes 1.3.0-beta.4.** The single source of "where Noir is right now." Update this whenever a slice ships or direction shifts — so no session loses the thread.

> **v1.3.0-beta.5 (on `develop`, pushed to origin — 2026-07-26 session):** the scaffold/TUI discovery → 8 sub-projects (SP-A…H) + an opus review fix wave, then validated end-to-end in the real project svc-academic-activity-go (which surfaced 4 more bugs → beta.2…beta.5). **SP-A** scaffold root-safety (the "noir init duplicates" bug fixed — `assertSafeRoot`) + already-init no-op (`ScaffoldResult.noop`) + `noir doctor` nested-`.noir` detection (incl. nested ignore files); **SP-B** branded banner (correct "NOIR" ANSI Shadow + smooth gradient-string) + host-aware home; **SP-C** regenerate conflict resolution (`onConflict` + `@clack` menu + `--force`); **SP-D** semantic dedup (`noir doctor --dedup`, S6 embeddings); **SP-E+SP-H** three-way managed-region merge (`noir sync --merge`, single + multi-region); **SP-F** content-hash dedup (`ScaffoldResult.identical`); **SP-G** `--json`/`--no-input` conflict-prompt fix. Validation patches: beta.2 (init no-op re-emitted skills), beta.3 (stack-aware ignore — no .npmignore/.prettierignore for Go), beta.4 (checkNestedNoir detects nested ignore files), beta.5 (banner "NOHA"→"NOIR" + gradient-string). All TDD; full repo green (**1158 tests**, 11 packages). svc-academic-activity-go cleaned (0 duplicate groups). CHANGELOG §1.3.0-beta.5; specs in `docs/superpowers/specs/2026-07-26-*-design.md`. **No remaining follow-ups.** Next: validate the beta in more real projects per host → promote to stable (`develop`→`main`, `node scripts/bump-version.mjs 1.3.0`, tag `v1.3.0` on `main`).
