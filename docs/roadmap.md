...
> **Un-released on `develop` (pushed to origin — 2026-07-26 session):** seven sub-projects (SP-A…G) from the scaffold/TUI discovery — **SP-A** scaffold root-safety (the "noir init duplicates" bug is fixed) + already-init no-op + `noir doctor` nested-`.noir` detection; **SP-B** branded banner + host-aware home; **SP-C** regenerate conflict resolution; **SP-D** semantic dedup (`noir doctor --dedup`); **SP-E** three-way managed-region merge (`noir sync --merge`); **SP-F** content-hash dedup (`identical`); **SP-G** `--json`/`--no-input` conflict-prompt fix — plus an opus review fix wave. All TDD; full repo green (1153 tests). CHANGELOG §Unreleased; specs in `docs/superpowers/specs/2026-07-26-*-design.md`. **Only remaining follow-up:** multi-region (CLAUDE.md) three-way merge.
