...
> **As of 2026-07-27. v1.4.0-beta.1 PUBLISHED on npm (dist-tag `beta`); the first published version of the 2026-07-26/27 runtime-polish work (the intermediate `1.3.0-beta.7`/`1.3.0-beta.8`/`1.4.0` tags failed CI on a `useColor()` leak under `CI=true` and were never published).** The single source of "where Noir is right now." Update this whenever a slice ships or direction shifts — so no session loses the thread.

> **Release-channel note:** the branch selects the npm dist-tag only (`develop` → `beta`, `main` → `latest`); maintainers choose the release version explicitly with `scripts/bump-version.mjs`.

> **Stable-release safety:** no stable `1.x` package release has been published to `@noir-ai/cli`'s `latest` tag, which remains at `1.0.0-beta.1`. The existing `v1.4.0` tag is a failed, unpublished release attempt; do not reuse it for a later stable release unless it is deliberately deleted first.

> **Correction to the release narrative below:** `1.4.0-beta.1` was selected explicitly; `develop` selected its npm `beta` dist-tag but did not add the prerelease suffix. Treat this note and the stable-release safety note as the current release guidance.

> **v1.4.0-beta.1 (on `develop`, PUBLISHED on npm dist-tag `beta`, 2026-07-27 overnight):** This explicit prerelease version shipped the TUI runtime policy, host handoff, the Ink `noir tui` MVP, idempotent scaffold and universal conflict work, output-design cleanup, and the CI color fix that unblocked publication. All 11 `@noir-ai/*` packages published with SLSA provenance; **1315/1315 tests** passed under `CI=true`. The failed `v1.3.0-beta.7`, `v1.3.0-beta.8`, and `v1.4.0` tags remain unpublished historical markers. **Next:** validate the beta in real projects per host, then merge `develop` into `main` without a release tag. A later stable release must use an unused version/tag or deliberately delete the failed `v1.4.0` tag first; richer TUI widgets and a `@huggingface/transformers` bump remain deferred.

> **v1.3.0-beta.8 (on `develop`, 2026-07-26 overnight — structural batch; tag pushed but CI FAILED, never published):** (1) **Idempotent scaffold** — `noir sync` on an unchanged tree writes NOTHING (managed-region content-hash dedup); ancestors seeded every run; `mergeManagedRegions` defaults TRUE (bare sync preserves in-region user edits; `--no-merge-regions` escape); bare init no-ops (incl. pre-1.3.0 legacy via `project.id`); hermetic `interactive` flag; latent newline-drift fix; stale multi-region comment fixed. (2) **Universal conflict contract** — every producer (incl. the 3 that blind-overwrote: skills `emitSkillsToDir` with `assertNotUserOwned`-guarded orphan cleanup, `workflow/artifacts`, `store/markdown`) routes through one seam; colored diff preview to stderr; apply-to-all (regenerate class); zdiff3 "merge with markers"; structured `--json` `conflicts[]`. (3) **Write-path semantic dedup** — init/create/sync surface near-duplicate host files as a non-blocking Replace/Mirror/Skip/Create recommendation (two-tier, `.noir/dedup-cache.json` content-hash cache, graceful embedder degradation — never blocks init). `init/sync/create` now return `ScaffoldResult`. **1263/1263 green** (was 1181). NEXT (this session): TUI runtime policy + host handoff + Ink `noir tui` MVP → 1.4.0-beta.1.

> **v1.3.0-beta.7 (on `develop`, 2026-07-26 overnight "runtime polish" session; tag pushed but CI FAILED, never published):** (1) **npm install deprecation warnings fixed at the source** — `better-sqlite3` `^12→^13` (N-API rewrite) removes `prebuild-install` entirely for Noir AND consumers; `engines.node` `>=20→>=22` (Node 20 EOL); `boolean` muted dev-side (`allowedDeprecatedVersions`); new "Deprecation warnings during install" troubleshooting doc; all Node-floor refs updated. (2) **Unified output design-system** — new `theme.ts` (palette + `badge()` + `useColor`/`terminalWidth`); `table()` rewritten to kill the default red headers (pre-color via picocolors, bypass `@colors/colors`) + responsive widths + wordWrap/truncate; `definitionList()`/`kv()` helpers; status/task/context/memory/doctor refactored to badges (`✓/⚠/✗`, red reserved for errors); honors `NO_COLOR`/`CLICOLOR_FORCE`/`NOIR_ACCESSIBLE`. `--json` byte-identical. +23 design-system tests; **1181/1181 green** (was 1158). NEXT (this session): scaffold idempotency + conflict contract + write-path dedup (→ beta.8), then TUI policy + host handoff + Ink `noir tui` MVP (→ 1.4.0-beta.1). Plan: `docs/superpowers/plans/2026-07-26-overnight-runtime-polish.md`.

> **v1.3.0-beta.6 (on `develop`, pushed to origin — 2026-07-26 session):** the scaffold/TUI discovery → 8 sub-projects (SP-A…H) + an opus review fix wave, then validated end-to-end in the real project svc-academic-activity-go (which surfaced 5 more bugs → beta.2…beta.6). **SP-A** scaffold root-safety (the "noir init duplicates" bug fixed — `assertSafeRoot`) + already-init no-op (`ScaffoldResult.noop`) + `noir doctor` nested-`.noir` detection (incl. nested ignore files); **SP-B** branded banner (correct "NOIR" ANSI Shadow + Midnight Cobalt gradient `#2c5282→#3b82f6→#7dd3fc` via gradient-string) + host-aware home; **SP-C** regenerate conflict resolution (`onConflict` + `@clack` menu + `--force`); **SP-D** semantic dedup (`noir doctor --dedup`, S6 embeddings); **SP-E+SP-H** three-way managed-region merge (`noir sync --merge`, single + multi-region); **SP-F** content-hash dedup (`ScaffoldResult.identical`); **SP-G** `--json`/`--no-input` conflict-prompt fix. Validation patches: beta.2 (init no-op re-emitted skills), beta.3 (stack-aware ignore — no .npmignore/.prettierignore for Go), beta.4 (checkNestedNoir detects nested ignore files), beta.5 (banner "NOHA"→"NOIR" + gradient-string), beta.6 (gradient → Midnight Cobalt). All TDD; full repo green (**1158 tests**, 11 packages). svc-academic-activity-go cleaned (0 duplicate groups). CHANGELOG §1.3.0-beta.6; specs in `docs/superpowers/specs/2026-07-26-*-design.md`. **No remaining follow-ups.** Next: validate the beta in more real projects per host → promote to stable (`develop`→`main`, `node scripts/bump-version.mjs 1.3.0`, tag `v1.3.0` on `main`).
...
- **v1.2.0-beta.1** first published the previously untagged v1.x capability work (K/R/I/P/S/X) alongside multi-host (S10) and the SDK/doctor remainder (S11) — see below (1089 tests).
- **v1.3.0-beta.6 PUBLISHED on npm** — the scaffold/TUI discovery (SP-A…H) + real-project validation across hosts (1158 tests; supersedes 1.2.0-beta.1 on the `beta` dist-tag).
- **v1.3.0-beta.7 / v1.3.0-beta.8 / v1.4.0 — pushed but CI FAILED, NEVER PUBLISHED** (a `useColor()` leak: under `CI=true` table headers got ANSI-wrapped and the responsive-table "fits 60/80/120 cols" test measured ANSI bytes as overflow). The work landed in 1.4.0-beta.1 instead.
- **v1.4.0-beta.1 PUBLISHED on npm** (2026-07-27) — all 11 `@noir-ai/*` packages, dist-tag `beta` + SLSA provenance; the runtime-polish work (install deprecation fixes, output design-system, idempotent scaffold, universal conflict contract, write-path dedup, TUI runtime policy, host handoff, Ink `noir tui` MVP, repo-wide label cleanup, CI color fix). **1315/1315 tests** under `CI=true`; current beta.
- **NEXT = validate 1.4.0-beta.1 in real projects per host**, then merge `develop`→`main` without a release tag while the beta remains current. A future stable promotion must use an unused stable version/tag (or deliberately delete the failed `v1.4.0` tag first); only a new `v*` tag on `main` causes CI to publish `latest`. `@noir-ai/cli`'s `latest` remains at 1.0.0-beta.1 until that promotion. Then: richer TUI widgets; bump `@huggingface/transformers` when transformers.js #1730/#1718 ship.

**v1.x capability slices (designed 2026-07-25; implemented in unpublished 1.1 work and first published in 1.2.0-beta.1):**
Design: `docs/specs/2026-07-25-v1x-capabilities-design.md`. Five capabilities extend one keystone refactor. **6/6 done; first published in 1.2.0-beta.1** (966 tests at the unpublished 1.1 milestone):
...
**All 6 v1.x capability slices (K/R/I/P/S/X) were implemented in the unpublished 1.1 work and first shipped in 1.2.0-beta.1.** Specs: `docs/superpowers/specs/2026-07-25-slice-{s-scaffold,x-integration}-design.md`. **Next-session playbook + full technical debt: `docs/v1x-next-session.md`.**
...
> Note: `develop` is at **1315 tests** at `v1.4.0-beta.1` (K/R/I/P/S/X + the v1.x debt batch + S10 multi-host + S11 + the runtime-polish work; lint→0); `main`/`@noir-ai/cli` `latest` stays at 1.0.0-beta.1 until a future release tag is published from `main`.
...
**Known v0 debt** is now consolidated in the **"v1.x backlog"** section below (S10 hosts, S11 distribution, daemon detach/auth, richer TUI widgets, in-process read-only fallback, tree-sitter chunking, OS keychain, prompt caching, streaming, god-file refactors, and more).
...
- **Distribution DONE + LIVE:** branch-based beta/stable release flow (`release.yml`, npm automation token + provenance), `scripts/install.sh` native installer, npm publish metadata on all 11 packages, release CI, and `docs/installation.md` + `docs/releasing.md`. **v1.0.0-beta.1 shipped 2026-07-25** (all 10 `@noir-ai/*`, dist-tag `beta`); stable promotion is a separate, deliberate `develop`→`main` + tag-on-`main` step (not done yet — `@noir-ai/cli` `latest` still resolves to 1.0.0-beta.1; the stable version must be unused because `v1.4.0` is already reserved by a failed tag).
...
### CLI / TUI
- richer Ink TUI widgets — the `noir tui` MVP dashboard shipped in 1.4.0-beta.1 (StatusBar/OutputPane/CommandInput/Header/Footer, lazy-loaded React 19); full-screen richer widgets are still deferred.
...
### Workflow
> **Resolved in v1.1.0-beta.1:** the S4 dual-source-of-truth collapse (W1 — `task.history` vs `audit:<id>` KV) and the vestigial checkpoint (W2 — wired into audit export). No workflow items remain deferred from the v1.0 cut.

### Context
> **Resolved in v1.1.0-beta.1:** kNN-only-hit snippet hydration (C1 — was an empty snippet reported as `mode:'hybrid'`).
...
### Toolchain / quality
> **Resolved in v1.1.0-beta.1:** stale-skill-dir cleanup on `noir sync` (T2 — removed/renamed builtin skills are now pruned; and 1.4.0-beta.1's universal conflict contract routes every producer through one `onConflict` seam with `assertNotUserOwned`-guarded orphan cleanup).
- `tsconfig.test.json` — piloted on `@noir-ai/cli` in v1.1.0-beta.1 (T1); the remaining 10 packages are still `src`-only.
...
### v1.x capabilities (K/R/I/P/S/X) — sub-items
> **All resolved in v1.1.0-beta.1 (2026-07-25):** K3 (skills-compiler generalization → `discoverIntegrations` + `integration.json` schema, landed in slice X); R4/R5 (`rules:` config block + `noir doctor` RULES.md budget check); P3/P4 (`draftPrd` + `prd:` config + `advance()` soft PRD gate); and lint → 0 (the 10 `noCommaOperator`/`noNonNullAssertion` warnings plus the red `useOptionalChain`/`useLiteralKeys` baseline, now GREEN). The only remaining X items are runtime verifications (not blockers): ClickUp `GET /list/{id}` `statuses` field; tag auto-create vs 400 (ClickApp-dependent). See the "Resolved in v1.1.0-beta.1" note above and `docs/CHANGELOG.md` §1.1.0-beta.1.
