...
> **v1.4.0-beta.1 (on `develop`, PUBLISHED on npm dist-tag `beta`, 2026-07-27 overnight):** (1) **TUI runtime policy** — bare `noir` is the documented primary UX; `--tui`/`--no-tui`/`--no-tips` global flags; no command hard-gated (every subcommand stays scriptable); every read-side command emits `{ok,data}` `--json`; new `docs/command-policy.md` + `docs/deprecation.md` (zero entries). (2) **Host handoff** — `noir handoff`/`wrap` emits a pasteable markdown artifact (STDOUT / `--write` gitignored / `--json`); `hostLaunchDirective(host)` single-source text-only directive (optional `HostAdapter.emitHandoff?`); `noir-wrap` skill graduated; execute→verify tip. (3) **Ink `noir tui` MVP** — lazy React-19 dashboard (StatusBar/OutputPane/CommandInput/Header/Footer); `/`-commands via the `home` dispatch seam; the lazy load preserves the `isMainModule` symlink guard (runtime-expression dynamic import + two-config tsup build; `dist/bin.js` 1890 lines, guard inline, 0 React refs); 12 ink-testing-library tests. (4) **Repo-wide cleanup** — ~627 internal tier/task labels stripped, 3 test files renamed, handoff CWD-path bug fixed. (5) **CI color fix (the release blocker)** — `useColor()` returns false under `CI=true` (`CLICOLOR_FORCE=1` still forces color); this is what unpinned 1.4.0-beta.1 after the `1.3.0-beta.7`/`1.3.0-beta.8`/`1.4.0` tags failed CI and never published. All 11 `@noir-ai/*` packages published with SLSA provenance. **1315/1315 green** under `CI=true` (was 1263 after the structural batch). Minor bump (the `>=22` engines floor lands as minor; the `develop`→`beta` channel is why 1.4.0 → 1.4.0-beta.1). NEXT: validate in real projects per host → promote to stable 1.4.0 (`develop`→`main`, tag `v1.4.0` on `main` → `latest`); richer TUI widgets; bump `@huggingface/transformers` when transformers.js #1730/#1718 ship (removes `boolean` for consumers); optionally delete the 3 unpublished failed tags (`v1.3.0-beta.7`/`beta.8`/`v1.4.0`) from the remote.
...
- **NEXT = validate 1.4.0-beta.1 in real projects per host**, then promote to stable **1.4.0**: merge `develop`→`main`, tag `v1.4.0` on `main` → CI derives `channel=stable` and publishes `--tag latest` (so `npm i @noir-ai/cli` resolves to 1.4.0 stable). `latest`/stable still points at 1.0.0-beta.1 until that promotion. Then: richer TUI widgets; bump `@huggingface/transformers` when transformers.js #1730/#1718 ship.
...
> Note: `develop` is at **1315 tests** at `v1.4.0-beta.1` (K/R/I/P/S/X + the v1.x debt batch + S10 multi-host + S11 + the runtime-polish work; lint→0); `main`/npm `latest` stays at 1.0.0-beta.1 until the `develop`→`main` promotion.
