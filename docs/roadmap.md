...
- 501/501 tests green; all acceptance gates verified; final whole-branch reviews = release-ready.
- Legacy plugin rebranded: marketplace `noir`, plugin `noir-workflow`.

**Next:**
- **S9 CLI/TUI home screen** — the **v1.0 capstone**: an interactive home screen over the daemon unifying store / workflow / context / memory / model. Spec next.
- **Then:** v1.0 release. S10 (cross-CLI hosts) / S11 (distribution + SDK) begin **v1.x**.

**Still missing for v1.0 (the MVP target) — by design, built slice-by-slice:**
- the interactive TUI home screen (S9). Cross-CLI hosts (S10) and distribution/SDK (S11) are v1.x.
