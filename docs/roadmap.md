...
> **As of 2026-07-28. `1.5.0` PUBLISHED on npm (dist-tag `latest`); `1.4.0-beta.2` PUBLISHED on npm (dist-tag `beta`).** Source version is `1.5.0` across all 11 packages.

**Built & shipped:**
- **v1.0 release-ready** — walking skeleton (S0+S2+S3), S1 stores, S4 SDD workflow engine, S5 builtin skills + compiler, S6 context management, S7 memory, S8 bounded model layer, S9 CLI/TUI home screen — all shipped in `1.0.0-beta.1`
- **v1.x capabilities (K/R/I/P/S/X)** — keystone managedBlock refactor + rules/ignore/PRD/scaffold/integrations layer — first published in `1.2.0-beta.1`
- **Multi-host (S10)** — 5 adapters (claude/agents-md/gemini/cursor/opencode) — `1.2.0-beta.1`
- **Scaffold idempotency + universal conflict contract** — `1.3.0-beta.1` through `1.3.0-beta.6`
- **Runtime polish** — output design-system, host handoff, Ink TUI MVP — `1.4.0-beta.1`
- **Release automation** — auto-prerelease versioning, version registry, smart release tooling — `1.4.0-beta.2` + `1.5.0` stable
- **11 packages, 34 skills (33 builtins + 1 integration), 1315 tests green**

**Next candidates:**
- Validate `1.5.0` stable in real projects per host
- Richer `noir tui` widgets (multi-pane, scrollback, in-dashboard conflict resolution)
- `@huggingface/transformers` bump when `transformers.js#1730`/`#1718` ship
- Optional polish items from handoff documents
...
**v1.2.0-beta.1 — S10 multi-host + S11 remainder (DONE on `develop`; 1089/1089 tests green).** Noir is now cross-CLI. A `resolveAdapter(host)` registry over the `HostId` enum (`claude` | `agents-md` | `gemini` | `cursor` | `opencode`) drives per-host emission via `--host` on `noir init`/`create`/`sync`. `claude` stays the default — a bare `noir init` is byte-identical to pre-multi-host (regression anchor, existing tests stay green). A shared `emitAgentsMd(ctx)` helper writes the byte-identical universal `AGENTS.md` (the 32-platform standard) for every host; hosts with a native context file (`claude`→`CLAUDE.md`, `gemini`→`GEMINI.md`) keep it primary and **do not duplicate** into `AGENTS.md`. Cursor skills compile to flat `.cursor/rules/<name>.mdc`; opencode emits `opencode.json` with a distinct `type`-tagged MCP shape (verified against opencode.ai docs). S11 remainder: `docs/sdk.md` (the per-package framework/library API) + a `noir doctor` `publish` check (advisory package-metadata validation). Locked decisions in [ADR-0004](decisions/0004-multi-host-adapters.md); spec [`docs/internal/specs/2026-07-25-s10-multihost-design.md`](internal/specs/2026-07-25-s10-multihost-design.md). **Deferred:** `qwen` and `agy` adapters (the universal `AGENTS.md` covers them in the meantime).
...
- **SDK / doctor remainder — DONE in v1.2.0-beta.1:** [`docs/reference/packages.md`](reference/packages.md) (the per-package framework/library API surface — "usable as a framework") + a `noir doctor` `publish` check (advisory package-metadata validation across all `packages/*`: name, version, non-empty `files`, and `bin` for the cli). **S11 is now fully resolved.** (Distribution is npm-native; there is no plugin marketplace to publish to.)
