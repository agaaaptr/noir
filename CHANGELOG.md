...
- **Display integrity: one shared display-width module measures every terminal string.** `@noir-ai/cli` gains `width.ts` (`displayWidth` / `truncateToWidth` / `padToWidth` / `truncateMiddle`), which strips ANSI SGR sequences, measures East-Asian wide and fullwidth characters as 2 columns, and truncates without splitting a surrogate pair or a grapheme cluster. It declares `string-width` and `strip-ansi` directly — both already in the install graph through Ink, so no new package enters the install. Every renderer now measures through it: Ink components, `output.ts` tables, the run-status line, banners. The defect it closes: a coloured badge had its ANSI bytes counted as visible width, so a table or a status line overran the terminal and wrapped.
- **Every width budget is derived from its container, never hard-coded.** The post-run overlay, the dashboard divider, and the footer take their cells from the same rule the palette already used (container width − border − padding), so a change to the shared panel cannot silently re-break a child. Ink text that must not wrap is either truncated to the budget or given an explicit `wrap` prop, chosen deliberately at each site.
- **A table never exceeds the terminal.** Table geometry is derived from the stream the table is written to, and when columns reach their floor and the row still does not fit, the row is truncated with a visible ellipsis rather than emitted over-wide. Path-like values truncate from the middle, so the leading segments and the final segment both survive.
- **`--quiet` now actually disables decoration.** The colour authority never saw the parsed flag, so a quiet run still emitted ANSI whenever `CLICOLOR_FORCE` was set or the stream was a TTY. `bin.ts`'s `preAction` bridges the flag to `NOIR_QUIET` (and deletes any ambient value on a run that did not ask for quiet — the same "output, not input" rule as `NOIR_NON_INTERACTIVE`), and `useColor()` reads it live. Quiet sits above `CLICOLOR_FORCE` in the decision chain; `NO_COLOR` still wins over everything. `--json` output is byte-unchanged and still ANSI-free.
- **`rules.enabled` now gates everything it claims to.** The switch gates emission of the working-rules seed **and** the host import of those rules **and** the migration refresh, instead of being read in one place while the rest carried on regardless.

### Added
- **Output hygiene subsystem** — a single tiered rule source (`packages/skills/src/hygiene.ts`, extending the predecessor-residue scanner rather than creating a parallel system) defines the prohibited patterns, their tier, and a human-readable rationale for each. Deterministic patterns fail (decorative banner comments, `// Step 1:` workflow narration, decorative emoji in code); judgement patterns warn only (long comments, abbreviations), so a false positive cannot block legitimate work. Enforced in three places that cannot drift: the skills quality gate (`validateSkill`/`lintSkill`, surfaced by `noir skills lint`), a `noir doctor` check, and this repository's own CI (`pnpm hygiene:gate`, which runs the same `scanOutputHygiene` the doctor reports). A file that must quote a prohibited shape declares the exemption with `<!-- noir-hygiene: exempt -->` (or the `//` form in source) above its first finding.
- **The `noir-code-hygiene` builtin skill** — the full guidance as Tell/Why/Fix entries, covering comment hygiene (decorative banners, restating the obvious, workflow narration, empty labels, stale comments), decorative icon use, verbosity, internal jargon, and unstated assumptions. The same rules reach every host through a host-neutral seed template, and the skill's evals assert against real output rather than only the expected-output file. The pack is now **27 builtins + 1 integration = 28 skills**.
- **A `400` for a misdirected workspace URL, and `405` with `Allow` for a known path with an unsupported method.** A project daemon serves one project at a bare `/mcp`, so a query-string URL — the workspace shape, `/mcp?p=…` — used to draw a bare `404 not found`, which says "path does not exist" when the real problem is that the caller addressed the wrong daemon. It now answers `400` with a body naming the flavour mismatch; `404` stays reserved for genuinely unknown paths.
- **`noir workspace status` reports liveness from a `/health` probe** rather than `pidAlive` alone, and **`noir workspace stop` proves ownership through `/health`** before it signals a process, so a stale record whose pid now belongs to an unrelated process cannot be signalled.

### Security & robustness
- **`.noir/.env` is owner-only after every command that can touch it** — `noir init`, `noir sync`, `noir init --upgrade` and `noir init --force` re-assert mode `0600` on a file that already exists, so a file left group/world-readable by an editor that saves by rename (or created before this contract) heals on the next command instead of drifting silently. The heal is reported once, and `noir doctor` reports the observed mode when it is still wrong.
- **The store database and its parent directory are created owner-only** (`0600` / `0700`), degrading to a no-op where the platform has no POSIX permission bits.
- **The permission diagnostic is honest.** It reports the observed mode with every permission bit evaluated and distinguishes "created before this contract" from "changed by something else", replacing the claim in `install-method.ts` that a `writeFileSync` mode "survives any umask" — a claim that was wrong, because the mode is umask-masked.
- **Join and leave preserve user wiring** — any `headers`, `headersHelper` or `env` a user added to the `noir` entry survives the entry rewrite.

### Upgrade steps
1. **Joined a workspace before this release? Run `noir init --upgrade`.** A project stamped below the current scaffold version reports drift in `noir doctor` until it runs; the `1.2.0 → 1.3.0` migration rewrites a legacy `type: 'http'` `noir` entry (the `?p=` URL) to the workspace-aware stdio bridge entry. `noir sync` and `noir init --force` no longer downgrade a joined repo.
2. **Nothing to do for the rest.** The `.noir/.env` mode heal, the owner-only store, and the `--quiet` fix apply on the next command; run `noir doctor` to see the observed mode if a `.noir/.env` is not `0600`.
3. **The hygiene gate is Noir's own.** It gates this repository's CI and the skill/template text Noir emits — it is not a prose linter for arbitrary user repositories.
...
### Changed
- **BREAKING — `.noir/.env` now wins over the ambient environment, for every key it defines.** The real environment becomes the fallback for the keys the file omits. This is a deliberate departure from the 12-factor / dotenv / Node `--env-file` convention (fill-only-unset), which is correct for ambient overrides but means a project's own declared configuration can never describe the project — a token exported from `~/.zshrc` silently shadowed the one in the project file, which was the live failure this fixes. **A user with the same key in both places observes a different effective configuration after upgrading.** Nothing is rewritten automatically: Noir cannot know whether a duplicated key is a mistake or a deliberate override. Instead the inversion announces itself — the loader emits one stderr line per key whose file value overrides an ambient one (naming the **key**, never a value), and `noir env` (`--json` too) reports which source won for every key. A genuine per-invocation override is a `run.profiles.<n>.env` entry (`noir run --profile`), which still merges **over** the file; there is no `VAR=value noir …` prefix level, because such a prefix arrives in `process.env` indistinguishably from the inherited environment (see ADR-0011).
- **BREAKING — the daemon record is per-project, and the HTTP transport now requires a token.** The single global `~/.noir/daemon.json` is replaced by `~/.noir/daemons/<projectId>.json`, so two projects on one machine no longer clobber each other's record (the three `wrongProject` guards this replaced are deleted — foreign-record access is impossible by construction). The legacy file is read exactly once by a self-deleting migration on the first daemon-touching command: a daemon from a previous version is SIGTERMed, then the file is removed. Any host MCP config written for a **workspace** (`type: 'http'`) must be re-established after upgrade, because its port and token change. See ADR-0010.
- **`noir init --upgrade` now emits every manifest mode except `mergeJson`.** `skipIfExists` keeps its create-only-if-absent semantics, so an upgrade backfills seeds added to the manifest since a project was initialized (`.noir/.env`, `.noir/.env.example`, `.noir/README.md`) while remaining structurally unable to modify a file the user owns. `mergeJson` (`.claude/settings.local.json`) stays excluded: its merge re-appends the `SessionStart` hook whenever the dedup substring is absent, so an upgrade emit would resurrect a hook the user deliberately removed. `CURRENT_SCAFFOLD_VERSION` moves `1.0.0 → 1.1.0` with the first real migration entry, and `--dry-run` reports the backfill as planned writes without touching disk.
- **`.noir/README.md` is a managed block, re-emitted on `noir sync` / `init --upgrade`** — a stale runtime map is worse than none. User notes outside the block survive.
- **`daemon.port` is honoured as a preference, not a demand.** A bind conflict degrades to an ephemeral port with a one-line warning rather than failing the command, and the record always names the port actually bound, so the record never lies.
- **Configuration doctrine consolidated onto `.noir/.env`.** The docs no longer rank `~/.claude/settings.json` as the most reliable placement; `.noir/.env` is the recommended home for project-scoped configuration and secrets, and it is the one placement that works in every launch mode (a GUI-launched host, CI, and launchd do not source shell rc files). See the new [Configuring a project with `.noir/.env`](docs/how-to/configure-env.md).

### Added
- **Per-project daemon records** — `~/.noir/daemons/<projectId>.json` (`packages/daemon/src/project-record.ts`), with `NOIR_DAEMON_DIR` as the isolation override. `NOIR_DAEMON_JSON` is now legacy-only (read once by the migration).
- **HTTP transport auth token** — the daemon mints a fresh 32-byte token per start, written at mode `0600` to `~/.noir/daemons/<scopeKey>.token` next to that identity's record and **before** the record (a client that can see the record can already read the secret — no 401 window on a fresh start). Enforced on `/mcp` with a constant-time compare; `/health` stays token-free. `noir daemon token` prints it for a host `headersHelper`; a workspace daemon gets the same treatment keyed by workspace name. stdio is unchanged — it has no network surface, and a header-delivered token is unsafe there. Does not defend against a same-uid process (it can read the 0600 file); host/origin validation stays.
- **Connect-first daemon activation on the project path** — `withDaemon` connects to the recorded port first and only spawns on `ECONNREFUSED`, with bounded backoff. A daemon now starts because a connection was genuinely attempted. The **workspace** path is exempt by design (stays probe-only, per ADR-0009 §11).
- **`noir init` creates `.noir/.env`** at mode `0600`, containing only comments (loading it yields an empty overlay, so behaviour is unchanged) — the file is now the recommended home for project secrets and exists after init instead of being an undocumented convention.
- **`.noir/README.md`** — the `.noir/` runtime map: what init just wrote, what appears later and which command creates it, and where to go next. Host-agnostic.
- **`.noir/.env.example` aligned with the reference** — the full variable set, grouped and commented, presenting the model provider key as an `apiKeyEnv:` NAME example rather than a hard-coded variable, plus `NOIR_PROFILE` and the update kill-switches, with a pointer to `environment.md`.
- **`noir env`** — which configuration is in effect and where each value comes from, with a per-key provenance table (`--json` emits the structured form). Reads `loadNoirEnv().sources`; names and redacted shapes only, **never a value**. Curated rather than a dump of `process.env`: every key the file defines, plus the Noir-relevant ambient names, plus ambient names the project's own config references.
- **Provenance rows in `noir doctor`** — which source supplies each resolved key, and the `noir-env` check reports the file's permissions.
- **Git-tracked `.noir/.env` refusal** — a file git tracks may have arrived with the clone, and under the new precedence it could redirect a credential the fallback supplies. The refusal is decided **before** parsing, so a refused file cannot warn about, shadow, or contribute a single key; the remedy printed is `git rm --cached .noir/.env` plus the gitignore entry. Untracked is trusted, and every git failure (no binary, not a repo, timeout, `safe.directory`, dubious ownership) degrades to trusted — failing closed would let one exotic setup silently disable a project's own env file.
- **`noir run` credential diagnostics broadened** — the auth branch now covers `ANTHROPIC_AUTH_TOKEN` / `ANTHROPIC_BASE_URL` shapes as well as a plain API key, keyed off the stream's `errorCategory`; when the credential came from `.noir/.env` the message names that file, so the advice is actionable ("unset it in the named source"). Running outside an initialized project emits one informational stderr line offering `noir init` (suppressed under `--json`, never fails the run). No message prints a credential value.
- **Managed ignore-list cleanup** — the vestigial `/.noir/*.sock`, `/.noir/daemon.pid`, and `/.noir/state/` entries are gone: no socket is ever created, the daemon pid lives in `~/.noir/daemons/`, and `.noir/state/` never existed.

### Security & robustness
- **`.noir/.env` injection deny-list** — process-injection and redirect names (`NODE_OPTIONS`, `NODE_PATH`, `LD_PRELOAD`, `DYLD_INSERT_LIBRARIES`, `npm_config_*`, `COREPACK_*`, and Noir's own plumbing names) are refused from `.noir/.env` with a one-line warning. `NOIR_DAEMON_DIR` is listed explicitly: the alternation is `(?:$|_)`-anchored, so `NOIR_DAEMON_JSON` does **not** cover it, and redirecting the daemon record directory is a hijack vector.
- **A git-tracked `.noir/.env` is refused outright** (see Added) — the exfiltration path the precedence inversion would otherwise open.

### Upgrade steps
1. **Run `noir init --upgrade`** — the recommended (and only) post-upgrade step. It backfills `.noir/.env`, `.noir/.env.example`, and `.noir/README.md` if they are missing, runs the `1.0.0 → 1.1.0` scaffold migration, and never modifies a file you own. `noir doctor`'s scaffold-drift row prompts it.
2. **If a key is set in both `.noir/.env` and your environment, expect the project file to win.** You will see one stderr line naming the key on the first command that loads the file. Run `noir env` to see every resolved key and its winning source. If you deliberately exported an override, remove the key from `.noir/.env` (or use a `run.profiles.<n>.env` entry when you want it per-invocation).
3. **Workspace members: re-establish the host MCP entry.** A `.mcp.json` written for a workspace daemon (`type: 'http'`) carries the old port, and the token is new — `noir workspace leave` then `noir daemon join <name>` rewrites it. `noir daemon token` prints the new project token; a host that supports `headersHelper` should store the command, not the secret.
4. **Nothing to do for a running daemon.** A daemon from a previous version is retired automatically on the first daemon-touching command; if it does not exit within 5s the command refuses with `kill <pid>` instructions rather than starting a second writer. Stopping it yourself first is fine.
...
### Added
- **Shell-alias resolution fallback** — when `noir run --command <name>` fails with ENOENT, Noir probes the user's interactive shell: PATH entries exported in rc files are re-spawned directly; zsh/bash/fish aliases and functions are bridged through the shell with the prompt passed **only as argv** (never shell-parsed). Guarded (win32/name-charset/shell), timeout + process-group kill, resolved paths cached.
- **Run profiles** — `.noir/config.yml` `run.profiles.<name> {binary, env, args}` + `run.defaultProfile`; select via `noir run --profile <name>`, `NOIR_PROFILE`, or the default. `env` supports `${VAR}` expansion and `null` deletion (no literal secrets in the committable config). `noir run --list-profiles` prints the registry; unknown names error listing the available ones. See `docs/how-to/host-profiles.md`.
...
### Security & robustness
- **Cross-project daemon isolation** — a daemon now records the project it serves (record + `/health`), and every reuse/probe/stop/status path verifies it: a daemon started for project A is never silently reused by project B (which previously read/wrote A's store). Pre-1.12 daemons are reused rather than orphaned.
- **`.noir/.env` injection deny-list** — a committed `.noir/.env` from an untrusted checkout can no longer set process-injection / redirect vars (`NODE_OPTIONS`, `LD_PRELOAD`, `npm_*`, `COREPACK_*`, `NOIR_NODE_DIST_URL`, `NOIR_SYSTEM_NODE_BIN`, …) that could achieve code execution via a spawned node child or a redirected managed-Node download.
- **FTS5 literal escaping + hit clamps** — store search treats user queries as literal phrases (no FTS5 syntax errors / operator injection) and clamps result limits; the indexer confines reads/writes to the project root (canonical paths), and a missing sqlite-vec binary degrades cleanly instead of crashing.
...
- **Async startup version check** — non-blocking, cached (`~/.noir/update-cache.json`), 24h interval default (configurable under `update:` in `.noir/config.yml`). Never blocks the CLI, never makes a paid call, silent under `--quiet`/CI/non-TTY.
