...
/** A fresh repo with `<repo>/.noir/.env` written but not yet staged. */
function writeEnvInRepo(prefix: string, body: string): string {
  const repo = mkTmp(prefix);
  execFileSync('git', ['init', '-q'], { cwd: repo, stdio: 'ignore' });
  mkdirSync(join(repo, '.noir'), { recursive: true });
  writeFileSync(join(repo, '.noir', '.env'), body, 'utf8');
  chmodSync(join(repo, '.noir', '.env'), 0o600); // no permission-advisory noise
  return repo;
}
