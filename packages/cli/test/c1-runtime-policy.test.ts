...
describe('C1 / B3 gap close — init/sync/create emit ScaffoldResult under --json', () => {
  ...
  it('noir sync --json emits the ScaffoldResult returned by sync() to stdout', async () => {
    vi.mocked(sync).mockResolvedValueOnce({
      ...
      migrationsRan: [],
      migrationConflicts: [],
      stack: { languages: ['typescript'], monorepo: false, frameworks: [], packageManager: 'pnpm' },
      projectId: 'p-test',
      fromVersion: null,
      toVersion: '1.3.0',
      host: 'claude',
  ...
  it('noir create --json emits the ScaffoldResult returned by create() to stdout', async () => {
    vi.mocked(create).mockResolvedValueOnce({
      ...
      migrationsRan: [],
      migrationConflicts: [],
      stack: { languages: ['typescript'], monorepo: false, frameworks: [], packageManager: 'npm' },
      projectId: 'p-create',
      fromVersion: null,
      toVersion: '1.3.0',
      host: 'claude',
  ...
  it('noir init (no --json) writes NO envelope to stdout — human output unchanged', async () => {
    ...
    vi.mocked(create).mockResolvedValueOnce({
      ...
      migrationsRan: [],
      migrationConflicts: [],
      stack: { languages: [], monorepo: false, frameworks: [], packageManager: null },
      projectId: 'p',
      fromVersion: null,
      toVersion: '1.3.0',
      host: 'claude',
