...
//
// The scan itself — which files are read, which directories are excluded, the
// caps — lives in ../hygiene-scan.ts, the single source of scope for both this
// doctor row and the CI gate (scripts/hygiene-gate.mjs). This command adds the
 ...
 * check is written for. Returns the structured scan for callers that want more
 * than the row.
 */
export function checkOutputHygiene(checks: CheckResult[], root: string): HygieneScanResult {
  ...
  checks.push({
    name: 'output hygiene',
    status: result.fail > 0 ? 'fail' : result.warn > 0 ? 'warn' : 'ok',
