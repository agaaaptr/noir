...
// (`readDaemonToken`; the scope key for a project daemon is its projectId).
// Every `/mcp` connect below sends it via `httpTransport`; `/health` is
// token-free, so the liveness probe and the daemon record are unaffected. The
// stdio transport never uses a token (no network surface, no header trust).
 ...
 * `scopeKey` is the daemon identity: a project daemon's projectId.
 */
...
export async function withWorkspaceDaemon<T>(
  ...
  try {
    ...
    const connected: Client = client;
    // The workspace daemon's own scope key lands with its transport auth
    // (Task 6 mirrors the project-daemon scheme using the workspace name).
    // Until then it is token-free, so no bearer is sent — and in particular the
    // CALLER's project token is never handed to a workspace server.
    await connectClient(connected, url, opts, null);
    return await fn(buildCaller(connected, opts));
  } finally {
    if (client) {
      await client.close().catch(() => {
