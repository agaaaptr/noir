...
// the daemon's MCP tool surface. This module is the single bridge from
...
// Streamable HTTP to that URL (127.0.0.1 only — the daemon never binds a public
// interface), initialize, and
...
//
// Failure handling (CLI exit-code contract): ANY failure to reach or use the
...
// the underlying cause on stderr; `--json` shapes the failure as the
...
//
// Auth: the HTTP transport requires a bearer token, regenerated by
...
// projectId, a workspace daemon's workspace NAME. Every `/mcp`
...
export interface DaemonClientOptions {
  /** Emit the daemon-down failure as a JSON `{ok,error}` envelope under `--json`. */
  readonly json?: boolean;
  /** Surface transport / connection detail on stderr under `--verbose`. */
...
export interface DaemonToolCaller {
   ...
   * failures throw a {@link NoirCliError} (exit 4); a tool's own logical
...
/** Daemon-down remediation hint. Stable across releases. */
...
/**
 * Map any daemon / transport failure onto the DAEMON_DOWN exit code. Always
...
function failDaemonDown(opts: DaemonClientOptions, cause: unknown): never {
  ...
  // Pass the PLAIN hint + opts and let `output.ts:fail` shape the single
 ...
 * onto exit 4.
 *
 * Ordering is CONNECT-FIRST. When `daemon.port` gives a stable
 ...
 * ensure-first ordering is retained verbatim (the connect still depends on the
 * ensure result).
 *
 ...
 * stall. A permanently dead port therefore ends in the exit-4 DAEMON_DOWN
...
async function resolveDaemon(
  ...
  // recovery and the exit-4 mapping applies (unlike the probe above).
 ...
 * on every start, so the header is resolved at CONNECT time — never
 ...
 *
 * {@link resolveDaemon} owns the CONNECT-FIRST ordering and returns
 ...
 * or unavailable embedder degrades reads to BM25-only); the memory engine
...
export async function withWorkspaceDaemon<T>(
  ...
  // A write/read against a workspace whose daemon is DOWN must fail
